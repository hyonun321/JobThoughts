...
const CardWrapper = styled.div<{
  ...
  @media (max-width: 780px) {
    width: ${({ mode }) => (mode === 'info' ? 'clamp(40px, 40vw, 100px)' : '90%')};
    height: ${({ mode }) => (mode === 'info' ? 'clamp(40px, 40vw, 100px)' : '50%')};
    flex-direction: ${({ mode }) => (mode === 'info' ? 'column' : 'row')};
    gap: ${({ mode }) => (mode === 'info' ? '0rem' : '1rem')};
    padding: ${({ mode }) => (mode === 'info' ? '0rem' : '1rem 1.25rem')};
  }
`;
